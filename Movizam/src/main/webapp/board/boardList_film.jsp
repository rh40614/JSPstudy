...
<ul class="tab_title">
	...
	<li class="on"><a href="<%=request.getContextPath()%>/board/boardList_film.do">영화게시판</a></li>
...
<span>| 영화게시판</span>
<p>최근에 감상하신 영화가 있으신가요? 영화를 추천해주세요</p>
...
if(pm.isPrev() == true){	
	out.print("<a href='"+request.getContextPath()+"/board/boardList_film.do?page="+(pm.getStartPage()-1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType() +"'>◀</a>");
}
%>
</td>
<td>
...
for(int i=pm.getStartPage(); i<= pm.getEndPage();i++){
	out.print("<a href='"+request.getContextPath()+"/board/boardList_film.do?page="+i+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>"+i+"</a>");
}
...
if(pm.isNext() && pm.getEndPage() >0 ){ 
	...
	out.print("<a href='"+request.getContextPath()+"/board/boardList_film.do?page="+(pm.getEndPage()+1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>▶</a>");
}%>
</td>
</tr>
...
<!-- 검색창 -->
<form name="frm" action="<%=request.getContextPath() %>/board/boardList_film.do"  method="post"> 
<table id="searchK"> 
<tr>
<td>
<select>
