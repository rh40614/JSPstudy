...
<meta charset="UTF-8">
<title>정보 관리</title>
<link rel="stylesheet" href="/Movizam/member/memberEditAdmin.css">
...
function editDoneFn(){
	...
	if($("#memberEmail").val()==""){
		...
		$("#email_check").html('');
																					...
																					//midx 도 같이 보내기
	fm.attr("action","<%=request.getContextPath()%>/member/memberEditAction.do?midx=<%=mv.getMidx() %>");
	fm.attr("method","post");
	fm.submit();
...
<nav id="nav">
<ul class="tab_title">
	<li class="on"><a href="<%=request.getContextPath()%>/member/memberInfo.do">내 정보 보기</a></li>
...
<form id="edit" class="edit" enctype="multipart/form-data" method="post">
<label><span>비밀번호</span>
<input type="password" name="memberPwd" id="memberPwd" maxlength="15" size="30" value="<%=mv.getMemberPwd() %>"></label>
<div id="pwd_check"></div>
<label><span>비밀번호</span>
<input type="password" name="memberPwd2" id="memberPwd2" placeholder="비밀번호를 확인해주세요" maxlength="15" size="30"></label>
<div id="pwd2_check"></div>
<label><span>닉네임</span>
<input type="text" name="memberNickname" id="memberNickname" size="30" value="<%=mv.getMemberNickname() %>"></label>
<div id="nickname_check"></div>
<label><span>이름</span>
<input type="text" name="memberName" id="memberName"size="30" value="<%=mv.getMemberName() %>"></label>
<div id="name_check"></div>


<label><span>생년월일</span>     
<!-- BIRTH_YY -->
<input type="text" name="birth_yy" id="yy" class="int" maxlength="4" size="10" value="<%=mv.getBirth_yy() %>">
<!-- BIRTH_MM -->
<select id="mm" class="int" name="birth_mm" >
<!--<option value="<%=mv.getBirth_mm()%>"></option>-->
...
<!-- BIRTH_DD -->
<input type="text" name="birth_dd" id="dd" class="int" maxlength="2"  size="10" value="<%=mv.getBirth_dd() %>">
</label>
<div id="birth_check"></div>      
       
<label><span>주소</span>
<input type="text" name="memberAdr" id="memberAdr" size="50" value="<%=mv.getMemberAdr() %>"></label>
<div id="adr_check"></div>
<label><span>전화번호</span>
<input type="text" name="memberPhone"  id="memberPhone" size="30" value="<%=mv.getMemberPhone() %>"></label>
<div id="phone_check"></div>
<label><span>이메일</span>
<input type="email" name="memberEmail" id="memberEmail" size="30px" value="<%=mv.getMemberEmail() %>"></label>
...
<% if(mv.getMemberImage() != null){		%>
	<img src="<%=request.getContextPath()%>/image/profile/<%=mv.getMemberImage() %>" alt="사진 불러오기 실패">
<%  } %>
<input type= "file" name="memberImage" value="<%=mv.getMemberImage() %>">
</label>


<button type="button" id="editDonebtn" onclick="editDoneFn()">수정완료</button>
